...
;	Mod F.V., 2007 at UF : make golden mean double precision.
;-

pro minF_bracket, xa,xb,xc, fa,fb,fc, FUNC_NAME=func_name, $
					POINT_NDIM=pn, DIRECTION=dirn

	goldm = double( sqrt(5) + 1 )/2	;golden mean factor to march with.
	...
	while NOT finite( fa ) do begin
		xa = xa/100.0
		fa = call_function( func_name, pn + xa * dirn )
	  endwhile

	while NOT finite( fb ) do begin
		xb = xb/100.0
		fb = call_function( func_name, pn + xb * dirn )
	  endwhile
