...
function ZBRENT, x1, x2, FUNC_NAME=func_name,    $
	...
	if ((fb LT 0) and (fa LT 0)) or ((fb GT 0) and (fa GT 0)) then begin
		message,"root must be bracketed by the 2 inputs",/INFO
		if abs( fb ) LT abs( fa ) then return,xb else return,xa
	   endif

	for iter = 1,maxit do begin

		if ((fb LT 0) and (fc LT 0)) or $
		   ((fb GE 0) and (fc GE 0)) then begin
			xc = xa
			fc = fa
			Din = xb - xa
			Dold = Din
