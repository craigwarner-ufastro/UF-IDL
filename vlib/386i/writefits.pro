PRO WRITEFITS,FILENAME,DATA,HEADER
;+
; NAME:
;	WRITEFITS
; CALLING SEQUENCE:
...
;
	IF NBYTES2 GT 0 THEN BEGIN
	   LINE = LINE * 0B
	   CASE TYPE OF
		7:	LINE(0) = BYTE(DATA,POS,NBYTES2)
		1:	LINE(0) = BYTE(DATA,POS,NBYTES2)
		2:	LINE(0) = BYTE(DATA,POS*2,NBYTES2)
		4:	LINE(0) = BYTE(DATA,POS*4,NBYTES2)
	        3:	LINE(0) = BYTE(DATA,POS*4,NBYTES2)
	        5:	LINE(0) = BYTE(DATA,POS*8,NBYTES2)
	   ENDCASE
	   FILE(R) = LINE		
	ENDIF	
;
