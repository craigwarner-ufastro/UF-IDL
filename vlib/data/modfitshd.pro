...
;      MODFITSHD
; PURPOSE:
;      Modify a FITS file by updating the header.
; EXPLANATION:
;      Update header in a specified FITS extension or primary HDU.
;    
;      The size of the supplied FITS header or data array does not
;      need to match the size of the existing header or data array.
;
; CALLING SEQUENCE:
;      MODFITS, Filename_or_fcb, Header [, EXTEN_NO =, EXTNAME= , ERRMSG=]
;
; INPUTS:
...
;       This version is just to modify the header, F.V.2020.
;-
 pro MODFITSHD, filename, header, EXTEN_NO = exten_no, ERRMSG = errmsg, EXTNAME = extname
...
; Check for filename input
   ...
   if N_params() LT 1 then begin                
      print,'Syntax - ' + $
        'MODFITS, Filename, Header [, EXTEN_NO=, EXTNAME=, ERRMSG= ]'
      return
   endif

   if N_elements( exten_no ) ne 1 then exten_no = 0
   if N_elements( Header ) LE 0 then Header = 0
   ...
   if (exten_no EQ 0) && (~keyword_set(EXTNAME)) then begin 
         if nheader GT 0 then $
             if strmid( header[0], 0, 8)  NE 'SIMPLE  ' then begin 
                 message = 'Input header does not contain required SIMPLE keyword'
                 goto, BAD_EXIT
             endif
   endif else begin
         if nheader GT 1 then $
             if strmid( header[0], 0, 8)  NE 'XTENSION' then begin 
              message = 'Input header does not contain required XTENSION keyword'
              goto, BAD_EXIT
              endif
   endelse
