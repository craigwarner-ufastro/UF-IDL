...
;
; INPUTS:
;	struct_From = structure array (or single) to copy from.
...
;	F.V.2011, do special case of From just 1 element to many elements.
;	F.V.2011, added keyword TAG_INDICES = indices of from tags to copy.
;-

pro copy_struct, struct_From, struct_To, NT_copied, Recur_Level,            $
						EXCEPT_TAGS  = except_Tags, $
						SELECT_TAGS  = select_Tags, $
						TAG_INDICES  = index_Tags, $
	...
	if (Recur_Level EQ 0) then begin	;check only at first user call.
           ...
           if (N_from GT N_to) then begin
              ...
              message," # elements (" + strtrim( N_to, 2 ) + ") in output TO structure",/INFO
              message," increased to (" + strtrim( N_from, 2 ) + ") as in FROM structure",/INFO
              struct_To = [ struct_To, replicate( struct_To[0], N_from-N_to ) ]

           endif else if (N_from LT N_to) and (N_from gt 1) then begin

              message," # elements (" + strtrim( N_to, 2 ) + ") in output TO structure",/INFO
              message," decreased to (" + strtrim( N_from, 2 ) + ") as in FROM structure",/INFO
	...
	wto = indgen( Ntag_to )
        ...
        if N_elements( index_Tags ) gt 0 then begin
           w = where( index_Tags ge 0 and index_Tags LT Ntag_from, nw )
           if( nw gt 0 ) then Tags_from = Tags_from[index_Tags[w]]
        endif
