...
regx = !P.region(2) - !P.region(0)
regy = !P.region(3) - !P.region(1)

if (regx gt 0) and (regy gt 0) then begin
	roffx = !P.region(0)
	roffy = !P.region(1)
 endif else begin
	regx = 1
...
idx = [(regx-margins[0]-margins[2])/cols $
    ,(regy-margins[1]-margins[3])/rows]	; normalized coords per plot
...
pos = [(col-1)*idx[0],(rows-row)*idx[1],col*idx[0],(rows-row+1)*idx[1]] $
  + [margins[0],margins[1],margins[0],margins[1]] + [roffx,roffy,roffx,roffy]
